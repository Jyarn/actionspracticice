...
app.get("/getAllNotes", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async (err, decoded) => {
            ...
            if (!cursor) {
                return res.status(404).json({ error: "Unable to retrieve notes." });
            }
            res.status(200).json({ response: await cursor.toArray() });
        });
    } catch (error) {
        res.status(500).json( {error: error.message });
    }
...
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
    try {
        ...
        jwt.verify(token, "secret-key", async(err, decoded) => {
            ...
            const collection = db.collection(COLLECTIONS.notes);
            const result = await collection.deleteOne({ username: decoded.username, _id: new ObjectId(noteId) });
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
    try {
        ...
        if ((!title && !content) || !ObjectId.isValid(noteId)) {
